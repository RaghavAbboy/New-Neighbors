...
app.post('/api/getNeighbors', function (req, res) {
	...
	var checkAndRespond = function (neighborhoodObj) {
		if(eventNumber === numNeighborhoods) {
...
geoCode(searchInfo.address)
  ...
  .then(function (neighborhoodObj) {
    // console.log('Neighborhoods fetched:',neighborhoodObj);
    if(Object.keys(neighborhoodObj).length === 0) { checkAndRespond({}); }

    return startChains(neighborhoodObj);
    // return getStreetAddresses(neighborhoodObj);
  });
...
//-----------------------------------------------------------------------------------
//Start a promise chain for each neighborhood from the neighborhood object
var startChains = function (neighborhoodObj) {
  var deferred = Q.defer();
  var neighborhoodArray = Object.keys(neighborhoodObj);

  numNeighborhoods = neighborhoodArray.length;
  neighborhoodObject = neighborhoodObj;

  //remove
  console.log('StartChains called.');
  console.log('Number of neighborhoods:',numNeighborhoods);

  //Start a promise chain/cluster for each neighborhood
...
var getStreetAddresses = function (neighborhoodObj) {
  ...
  var makeRequest = function (neighborhood) {
    ...
    .then(function (tuple) {
      ...
      numEvents++;
      if(numEvents === numNeighborhoods) { deferred.resolve(neighborhoodObj); }
      else { numEvents++; }
    });
  }
